...
FrameBufferSource::FrameBufferSource(const FramebufferCreateInfo& frameBufferCreateInfo)
{
...
{
	m_swapChainFramebuffers.resize(frameBufferCreateInfo.swapChainImageViews.size());//初始化大小与交换区图像的数量一致
	...
	for (size_t i = 0; i < frameBufferCreateInfo.swapChainImageViews.size(); i++) {
		...
		if (vkCreateFramebuffer(frameBufferCreateInfo.device, &framebufferInfo, nullptr, &m_swapChainFramebuffers[i]) != VK_SUCCESS) {
